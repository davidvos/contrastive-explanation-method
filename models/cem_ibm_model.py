...
class AEADEN:
    def __init__(
        ...
        self.delta_k = torch.zeros(shape, dtype=torch.double) # adv_img
        self.y_k = torch.zeros(shape, dtype=torch.double) # adv_img_s
        ...
        for s in range(self.n_searches):
            ...
            for iteration in range(1, self.MAX_ITER+1):
                ...
                if self.loss_overall < self.best_g:
                    self.best_delta = self.delta_k.clone()
                    self.best_y = self.y_k
                    self.best_g = self.loss_overall

                if self.loss_alien_sample_s == 0:
    ...
    def FISTA(self, step):
        self.step_tracker = step
        self.zt = step / (step + 3)

        self.shrinkage()

        condition4 = torch.gt(torch.sub(self.delta_new, self.orig_img), 0.0).type(torch.double) #line 73
        condition5 = torch.le(torch.sub(self.delta_new, self.orig_img), 0.0).type(torch.float) #line 74

        if self.mode == "PP":
            self.delta_new = torch.mul(condition5, self.delta_new) + torch.mul(condition4, self.orig_img) # assign_adv_img
        elif self.mode == "PN":
            self.delta_new = torch.mul(condition4, self.delta_new) + torch.mul(condition5, self.orig_img) # assign_adv_img

        self.y_k_new = self.delta_new + torch.mul(self.zt, (self.delta_new - self.delta_k)) # assign_adv_img_s

        condition6 = torch.gt(torch.sub(self.y_k_new, self.orig_img), 0).type(torch.float)
        condition7 = torch.le(torch.sub(self.y_k_new, self.orig_img), 0).type(torch.float)

        if self.mode == "PP":
            self.y_k_new = torch.mul(condition7, self.y_k_new) + torch.mul(condition6, self.orig_img)
        elif self.mode == "PN":
            self.y_k_new = torch.mul(condition6, self.y_k_new) + torch.mul(condition7, self.orig_img)

        # print(f"condition shape: {condition7.shape} and sum shape: {(condition7 * self.y_k_new).shape}")

        # print(f"y_k shape: {self.y_k.shape} and new shape: {self.y_k_new.shape}")
    ...
    def shrinkage(self):
        condition1 = torch.gt(torch.sub(self.y_k, self.orig_img), self.beta).type(torch.float) # cond1 line 66 checks if all entries of adv_img_s - orig img are bigger than beta
        condition2 = torch.le(abs(torch.sub(self.y_k, self.orig_img)), self.beta).type(torch.float) # cond2 line 67
        condition3 = torch.lt(torch.sub(self.y_k, self.orig_img), -self.beta).type(torch.float)  #cond3 line 68

        upper = torch.min(torch.sub(self.y_k, self.beta), torch.tensor([0.5], dtype=torch.double)) #line 69
        lower = torch.max(torch.add(self.y_k, self.beta), torch.tensor([-0.5], dtype=torch.double)) #line 70

        self.delta_new = torch.mul(condition1, upper) + torch.mul(condition2, self.orig_img) + torch.mul(condition3, lower) #line 71

        # print(f"shape new delta: {self.delta_new.shape} condition shape: {condition1.shape}")


    def fn_loss(self):
        self.delta_img = self.orig_img.view(-1,1,28,28) - self.delta_k
        self.y_img = self.orig_img.view(-1,1,28,28) - self.y_k
        # print(f"img size {self.delta_img.shape} orig {self.orig_img.shape} orig view {self.orig_img.view(-1,1,28,28).shape} delta: {self.delta_k.shape}")


        if self.mode == "PP":
            self.img2label_score   = self.classifier.forward_no_sm(self.delta_img.view(-1,1,28,28).float())
            self.img2label_y_score = self.classifier.forward_no_sm(self.y_img.view(-1,1,28,28).float())
        elif self.mode == "PN":
            self.img2label_score   = self.classifier.forward_no_sm(self.delta_k.view(-1,1,28,28).float())
            self.img2label_y_score = self.classifier.forward_no_sm(self.y_k.view(-1,1,28,28).float())
        ...
        if self.loss_alien_sample_s == 0:
            self.delta_tracker.append(self.delta_k.clone())
            # print(f"{self.step_tracker}: max delta: {torch.max(self.delta_k)} min delta: {torch.min(self.delta_k)} mean delta: {self.delta_k.mean()}\n max non target: {self.max_nontarget_score} \n target: {self.target_score}\n target s: {target_score_s}\n max non target y: {max_nontarget_y_score}\n")
    ...
    def compute_g(self):
        ...
        self.l1_dist_s = torch.sum(torch.abs(self.y_img), dim=[1,2,3])

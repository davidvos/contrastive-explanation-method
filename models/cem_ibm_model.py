...
class AEADEN:
    ...
    def FISTA(self, step):
        ...
        condition4 = torch.gt((self.delta_new - self.orig_img), 0.0).type(torch.double) #line 73
    ...
    def shrinkage(self):
        ...
        upper = torch.min((self.y_k - self.beta), torch.tensor([0.5], dtype=torch.double)) #line 69
        lower = torch.max((self.y_k - self.beta), torch.tensor([-0.5], dtype=torch.double)) #line 70
    ...
    def compute_g(self):
        ...
        self.AE.double()
        if self.mode == "PP":
            self.AE_loss   = self.gamma * torch.pow(torch.norm(self.AE(self.delta_img.view(-1,1,28,28))-self.delta_img), 2)
            self.AE_loss_s = self.gamma * torch.pow(torch.norm(self.AE(self.delta_img.view(-1,1,28,28))-self.y_img), 2)

        elif self.mode == "PN":
            self.AE_loss   = self.gamma * torch.pow(torch.norm(self.AE(self.delta_k.view(-1,1,28,28))-self.delta_k), 2)
            self.AE_loss_s = self.gamma * torch.pow(torch.norm(self.AE(self.y_k.view(-1,1,28,28))-self.y_k), 2)

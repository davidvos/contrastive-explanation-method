...
class MNIST(Dataset):
    ...
    def __init__(self, download=False, batch_size=4, shuffle=True, binary=True):
        ...
        self.train_data = datasets.MNIST(
            root='./cem/datasets/data/mnist', train=True, download=download, transform=transform)
        self.test_data = datasets.MNIST(
            root='./cem/datasets/data/mnist', train=False, download=download, transform=transform)
        ...
        # Create a non-shuffled dataloader with a batch size of 1 for the train and test set.
